...
import { ArrowRight, ArrowUpRight, CheckCircle2, Filter as FilterIcon, Inbox, Mail, MessageCircle, Phone, Plus, Send, Sparkles, TrendingUp, Users } from "lucide-react";
import { formatDistanceToNow } from "date-fns";
import { Channel, LeadStatus, type Contact } from "@/features/contacts/interfaces/contact.interface";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <PendingDraftsCard items={pendingGroups} isLoading={draftsLoading} />
      </div>
...
function HeroBanner({ greeting, name, newThisWeek }: { greeting: string; name: string; newThisWeek: number }) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-surface p-6 sm:p-8">
      ...
      <div aria-hidden className="absolute -bottom-32 -left-12 size-72 rounded-full bg-fuchsia-500/15 blur-3xl" />
...
function HeroActions() {
  ...
  return (
    <div className="flex flex-wrap gap-2 sm:shrink-0">
      <Button variant="secondary" onPress={() => navigate(Routes.dashboard.leads_directory)}>
...
const KPI_TONE: Record<"accent" | "success" | "warning" | "fuchsia", { ring: string; bg: string; text: string }> = {
  ...
  const inner = (
    <div className="bg-surface rounded-xl border border-border p-5 flex flex-col gap-3 hover:border-accent/40 transition-colors h-full">
      <div className="flex items-center justify-between">
        ...
        <span className={cn("inline-flex size-9 items-center justify-center rounded-lg", tone.bg, tone.text)}>
...
function PipelineDistribution({ stats, isLoading }: { stats: DashboardStats | undefined; isLoading: boolean }) {
  const navigate = useNavigate();
    ...
    <section className="bg-surface rounded-xl border border-border p-6">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div>
          ...
          <p className="text-xs text-muted">How your contacts move through the funnel.</p>
        </div>
        <Button size="sm" variant="tertiary" onPress={() => navigate(Routes.dashboard.contacts)}>
          Open pipeline
        ...
        <>
          <div className="mt-5 flex h-3 w-full overflow-hidden rounded-full bg-surface-secondary">
            {statuses.map((s) => {
              ...
              return <div key={s} className={cn(STATUS_BAR_COLOR[s], "h-full")} style={{ width: `${pct}%` }} title={`${STATUS_LABEL[s]}: ${count}`} />;
            })}
          </div>
              ...
              return (
                <div key={s} className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={cn("size-2 rounded-full shrink-0", STATUS_BAR_COLOR[s])} />
                    <span className="text-sm text-foreground truncate">{STATUS_LABEL[s]}</span>
                  </div>
...
function TopContactsCard({ contacts, isLoading }: { contacts: Contact[]; isLoading: boolean }) {
  const navigate = useNavigate();
  return (
    <section className="bg-surface rounded-xl border border-border p-6 lg:col-span-2">
      <div className="flex items-center justify-between gap-2 flex-wrap mb-4">
        <div>
          <h2 className="text-sm font-semibold text-foreground">Top scored contacts</h2>
          <p className="text-xs text-muted">Where your AI rates the best fit.</p>
        </div>
        <Button size="sm" variant="tertiary" onPress={() => navigate(Routes.dashboard.contacts)}>
          View all
          <ArrowRight className="size-3.5" />
      ...
      {isLoading ? (
        ...
        <ul className="divide-y divide-border">
          {contacts.map((c) => (
            <li key={c.uuid}>
              <Link to={Routes.dashboard.contacts_detail.replace(":uuid", c.uuid)} className="flex items-center gap-3 py-3 hover:bg-surface-secondary/40 -mx-2 px-2 rounded-md transition-colors">
                <Avatar name={c.name} />
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-foreground truncate">{c.name ?? "Unnamed"}</p>
...
function PendingDraftsCard({ items, isLoading }: { items: DashboardPendingDraftGroup[]; isLoading: boolean }) {
  return (
    <section className="bg-surface rounded-xl border border-border p-6">
      <div className="flex items-center justify-between gap-2 flex-wrap mb-4">
        <div>
          <h2 className="text-sm font-semibold text-foreground">Drafts to send</h2>
      ...
      {isLoading ? (
        ...
        <ul className="space-y-2">
          {items.map(({ contact, drafts }) => {
            ...
            return (
              <li key={contact.uuid}>
                <Link to={Routes.dashboard.contacts_detail.replace(":uuid", contact.uuid)} className="flex items-center gap-3 rounded-lg border border-border bg-surface-secondary/40 p-3 hover:border-accent/40 transition-colors">
                  <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {channels.map((ch) => {
                      ...
                      return (
                        <span key={ch} title={ch} className="inline-flex size-6 items-center justify-center rounded-md bg-background text-muted">
...
function FiltersCard({ filters, isLoading }: { filters: Filter[]; isLoading: boolean }) {
  const navigate = useNavigate();
  return (
    <section className="bg-surface rounded-xl border border-border p-6">
      <div className="flex items-center justify-between gap-2 flex-wrap mb-4">
        <div>
          ...
          <p className="text-xs text-muted">Saved hunts that pull leads automatically.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button size="sm" variant="tertiary" onPress={() => navigate(Routes.dashboard.filters)}>
            View all
            <ArrowRight className="size-3.5" />
          </Button>
...
function FilterRow({ filter }: { filter: Filter }) {
  ...
  return (
    <li>
      <Link to={Routes.dashboard.filters_detail.replace(":uuid", filter.uuid)} className="flex items-center gap-3 rounded-lg border border-border p-3 hover:border-accent/40 transition-colors h-full">
...
function Avatar({ name }: { name: string | null }) {
  const initials =
    (name ?? "")
      ...
      .join("")
      .toUpperCase() || "?";
  return <span className="inline-flex size-9 shrink-0 items-center justify-center rounded-full bg-accent/15 text-accent text-xs font-semibold">{initials}</span>;
}
