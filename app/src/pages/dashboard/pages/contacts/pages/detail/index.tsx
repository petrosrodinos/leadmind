            ...
            <Section title="Public enrichment summary">
                ...
                Object.keys(contact.lead.enrichment_data).length > 0 ? (
                    <pre className="text-xs text-foreground whitespace-pre-wrap break-words font-mono p-3 rounded-lg bg-surface-secondary border border-border">
                        {JSON.stringify(contact.lead.enrichment_data, null, 2)}
                    </pre>
                ) : (
...
function CrmTab({ contact }: TabContactProps) {
    ...
    const handleTagsChange = (next: string[]) =>
        updateTags.mutate({ uuid: contact.uuid, tags: next });
        ...
        <div className="flex flex-wrap items-center gap-1.5">
            {tags.map((tag, idx) =>
                editingIdx === idx ? (
                    <span
                        key={`edit-${idx}`}
                        className="inline-flex items-center gap-1 rounded-md bg-surface-secondary border border-border px-2 py-0.5"
                    >
                        <Input
                            ref={editInputRef}
                            value={editingValue}
                    ...
                    <Chip key={tag} size="sm" variant="soft">
                        ...
                        <button
                            ...
                            disabled={disabled}
                            onClick={() => remove(tag)}
                            className="ml-1 text-muted hover:text-danger disabled:opacity-50"
            ...
            {tags.length === 0 && !adding && (
                <span className="text-xs text-muted italic">
                    Click on a tag to edit, X to remove.
                </span>
            )}
        </div>
    );
