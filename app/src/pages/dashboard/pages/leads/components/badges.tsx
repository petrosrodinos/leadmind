...
export function ScoreBadge({ score }: { score: number | null | undefined }) {
    if (score == null) {
        return (
            <span className="inline-flex items-center justify-center min-w-9 h-6 rounded-md bg-surface-secondary text-muted text-xs font-medium px-2">
                —
            </span>
        );
    }
    ...
    return (
        <span
            className={`inline-flex items-center justify-center min-w-9 h-6 rounded-md text-xs font-semibold px-2 ${tone}`}
        >
            {score}/10
        </span>
    );
