...
export function StepMessage({
    ...
    const composerChannels = useMemo(
        () => channels.filter((c) => c === Channel.EMAIL || c === Channel.SMS),
        [channels],
    );
    ...
    return (
        <div className="space-y-3">
            <p className="text-sm text-muted max-w-2xl">
                One template per channel. The AI panel helps draft, improve, shorten, or change
                tone — the output goes straight into the editor below for you to tweak before
                sending.
            </p>
            <MessageComposer
